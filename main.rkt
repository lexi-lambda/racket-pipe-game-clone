...
(struct world (grid tile-offsets))
    ...
    (for/fold ([grid (world-grid state)]
      ...
      (define-values (new-grid new-offsets)
        (if (in-radius? (exact->inexact mx)
            ...
            (values grid (make-immutable-hasheq '()))))
...
(big-bang
 (world starting-tile-grid (make-immutable-hasheq '()))
 [name "WIP Game Clone"]
 [to-draw (λ (state) (pict->bitmap (world->pict state)))]
